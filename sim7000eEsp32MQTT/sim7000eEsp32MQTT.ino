...
#define FONA_TX 16 // Microcontroller RX
#define FONA_RX 17 // Microcontroller TX
...
void loop() {
  if (firstTime || millis() - timer > samplingRate * 1000UL) {
    ...
    // only for testing.
    battLevel = 5.0; // Get voltage in mV
...
//    dtostrf(altitude, 1, 1, altBuff);
    ...
    if (! fona.MQTT_connectionStatus()) {
      ...
      fona.MQTT_setParameter("mqtt.cmmc.io", MQTT_SERVER, MQTT_PORT);
      // Set up MQTT username and password if necessary
      fona.MQTT_setParameter("", MQTT_USERNAME);
      fona.MQTT_setParameter("", MQTT_PASSWORD);
